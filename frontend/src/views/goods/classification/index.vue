    ...
    <a-card title="商品カテゴリ">
      <a-row gutter="16">
        ...
        <div style="margin-bottom: 12px; float: right;">
          <a-button type="primary" icon="plus" style="margin: 0 8px;" @click="openFormModal(form)">カテゴリを追加</a-button>
        </div>
      </a-row>
  ...
  methods: {
    ...
    exportExcel() {
      goodsClassificationExport(this.searchForm).then(resp => {
        exportExcel(resp.data, '商品カテゴリー一覧');
      }).catch(err => {
    ...
    downloadTemplate () {
      goodsClassificationTemplate().then(resp => {
        exportExcel(resp.data, '商品カテゴリインポートテンプレート');
      }).catch(err => {
        this.$message.error(err.response.data.error);
      });
    },
