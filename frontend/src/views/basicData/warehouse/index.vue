    ...
    <a-card title="入庫">
      <a-row gutter="16">
        ...
        <div style="margin-bottom: 12px; float: right;">
          <a-button type="primary" icon="plus" style="margin: 0 8px;" @click="openFormModal(form)">入庫を追加</a-button>
        </div>
      </a-row>
    ...
    methods: {
      ...
      downloadTemplate () {
        warehouseTemplate().then(resp => {
          exportExcel(resp.data, '入庫マスタインポートテンプレート');
        }).catch(err => {
          this.$message.error(err.response.data.error);
        });
      },
